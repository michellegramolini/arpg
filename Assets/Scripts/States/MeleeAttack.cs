...
public class MeleeAttack : State
{
    ...
    private void Animate()
    {
        ...
        {
            _player.animationState.SetAnimationState("kaia_melee_up");
        }
        else if (_startFacingDir == Vector2.down)
        {
            _player.animationState.SetAnimationState("kaia_melee_down");
        }
        // TODO: could handle this at the facingDirection vector level
        else if (_startFacingDir.x > 0f)
        {
            _player.animationState.SetAnimationState("kaia_melee_right");
        }
        else
        {
            _player.animationState.SetAnimationState("kaia_melee_left");
        }
    }
    ...
    private IEnumerator MeleeSwitch()
    {
        ...
        yield return new WaitForSeconds(_player.animationState.GetClipLength("kaia_melee_right") - 0.1f);
        _melee = false;
    }
